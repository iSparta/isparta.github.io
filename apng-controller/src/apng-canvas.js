...
(function() {
        ...
        if (cssCanvasEnabled) {
            ...
            if (img.hasAttribute("data-is-apng")) {
                ...
                Animation.createFromUrl(img.src).done(function() {
                    ...
                    d.resolve(urlToAnimation[img.src]);
                }).fail(function() {
    ...
    var tick = function(t) {
        ...
        for (var i = 0; i < allAnimations.length; i++) {
            ...
            while (anim.isActive && anim.nextRenderTime <= t){
                anim.playing(anim.nowFrame);
                anim.renderFrame(t);
            } 
        }
    ...
    var Animation = function() {
        ...
        this.play=function(){
            ...
            this.isActive=true;
            playto_f=-1;
            var delay = this.frames[this.nowFrame].delay;
            this.nextRenderTime=T+delay;
            d.resolve();
            return d.promise();
        };
        this.goto=function(f){
            var d=new Deferred();
            this.stop();
            playto_f=-1;
        ...
        var playToDeferred;
        this.playto=function(f){
            playToDeferred=new Deferred();
            this.play();
            playto_f=f;
            return playToDeferred.promise();
        };
